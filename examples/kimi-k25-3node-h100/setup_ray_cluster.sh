#!/bin/bash
# Ray cluster setup for Kimi-K2.5 BF16 Eagle3 3-node training
#
# Node layout:
...
LOG_FILE="$LOG_DIR/kimi25_3node_ray_${NODE_ROLE}_${TIMESTAMP}.log"
...
echo "=============================================="
echo "Kimi-K2.5 BF16 3-Node Ray Cluster"
...
case "$NODE_ROLE" in
  head)
    ...
    echo "Next steps:"
    echo "  Worker nodes: HEAD_IP=$LOCAL_IP NODE_ROLE=worker bash examples/kimi-k25-3node-h100/setup_ray_cluster.sh"
    echo "  Training:     bash examples/kimi-k25-3node-h100/run.sh"
    ;;
