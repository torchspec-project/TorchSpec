#!/bin/bash
# Launch torchspec.train_entry for Kimi-K2.5 BF16 Eagle3 3-node training
#
# Run this on the head node AFTER the Ray cluster is fully ready.
# See examples/kimi-k25-3node-h100/setup_ray_cluster.sh to set up the cluster first.
#
# Node layout:
...
#
# Usage:
#   bash examples/kimi-k25-3node-h100/run.sh [extra_overrides...]
#
...
CONFIG_FILE="${CONFIG_FILE:-$ROOT_DIR/configs/sglang_kimi_k25_3node.yaml}"

LOG_DIR="$ROOT_DIR/running_logs"
mkdir -p "$LOG_DIR"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
LOG_FILE="$LOG_DIR/kimi25_3node_train_${TIMESTAMP}.log"
exec > >(tee -a "$LOG_FILE") 2>&1
echo "Logging to: $LOG_FILE"

echo "=============================================="
echo "Kimi-K2.5 BF16 3-Node Training"
echo "=============================================="
...
if ! ray status &>/dev/null; then
  ...
  echo "  NODE_ROLE=head   bash examples/kimi-k25-3node-h100/setup_ray_cluster.sh"
  echo "  HEAD_IP=<node0_ip> NODE_ROLE=worker bash examples/kimi-k25-3node-h100/setup_ray_cluster.sh"
  exit 1
fi
